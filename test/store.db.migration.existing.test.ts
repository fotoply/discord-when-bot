...
describe('store/db migration when view_mode already exists', () => {
  it('no-ops when re-imported on the same DB path', async () => {
    vi.resetModules();
    const tmp = path.join(process.cwd(), 'test-data', `when.test.reimport.${Date.now()}.db`);
    ...
    const cols1 = db.prepare('PRAGMA table_info(polls)').all() as Array<{ name: string }>;
    ...
    const cols2 = db2.prepare('PRAGMA table_info(polls)').all() as Array<{ name: string }>;
