...
describe('Polls hydration and persistence', () => {
    ...
    it('hydrates a poll from the database when not present in memory', () => {
        const poll = Polls.createPoll({channelId: 'c-hyd', creatorId: 'creatorH', dates: ['2025-08-30']});
    ...
    it('setMessageIdAndChannel persists changes to DB and hydrates correctly', () => {
        const poll = Polls.createPoll({channelId: 'c-setmsg', creatorId: 'creatorM', dates: ['2025-08-30']});
        ...
        const row = db.prepare('SELECT channel_id AS channelId, message_id AS messageId FROM polls WHERE id = ?').get(poll.id) as any;
        expect(row.channelId).toBe('new-chan');
        expect(row.messageId).toBe('msg-123');
    });

    it('allOpen returns hydrated polls', () => {
        const poll = Polls.createPoll({channelId: 'c-allopen', creatorId: 'co', dates: ['2025-08-30']});
