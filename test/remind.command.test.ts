...
describe('Remind command', () => {
  ...
  it('shows config when enabled=show or start_time=show provided', async () => {
    ...
    await RemindCommand.prototype.chatInputRun.call(thisArg, showEnabled as any);
    let content = (showEnabled.reply.mock.calls[0]![0] as any).content as string;
  ...
  it('clamps interval_hours to at least 1', async () => {
    ...
    await RemindCommand.prototype.chatInputRun.call(thisArg, zeroInterval as any);
    const content = (zeroInterval.reply.mock.calls[0]![0] as any).content as string;
  ...
  it('registerApplicationCommands registers builder and respects GUILD_ID', async () => {
    ...
    RemindCommand.prototype.registerApplicationCommands.call({ name: 'remind', description: 'desc' }, registry);
    ...
    const chain: any = {
      ...
      addSubcommand: vi.fn((fn: any) => { fn(chain); return chain; }),
      addStringOption: vi.fn((fn: any) => { fn({ setName: vi.fn().mockReturnThis(), setDescription: vi.fn().mockReturnThis(), setRequired: vi.fn().mockReturnThis(), addChoices: vi.fn().mockReturnThis() }); return chain; }),
      addIntegerOption: vi.fn((fn: any) => { fn({ setName: vi.fn().mockReturnThis(), setDescription: vi.fn().mockReturnThis(), setRequired: vi.fn().mockReturnThis() }); return chain; }),
    };
    builderFn(chain);
