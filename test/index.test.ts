...
describe("reminder logic", () => {
  it("posts one channel message pinging non-responders and deletes old reminder", async () => {
    ...
    const client = {
      channels: { fetch: vi.fn(() => Promise.resolve(textChannel)) },
    } as any;
    ...
    const sentArg = (
      (sendMock.mock.calls as unknown as any[])[0] as any[]
    )[0] as any;
    ...
    const hadRemindersLog = (logSpy.mock.calls as any[]).some(
      (args) =>
        args[0] === "[reminders]" &&
        /sent\s+\d+\s+reminder message\(s\)\s+for poll/.test(String(args[1])),
    );
