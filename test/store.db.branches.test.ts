...
describe.sequential('store/db branches', () => {
  ...
  it('uses default db path when WHEN_DB_PATH is not set (RHS of ||)', async () => {
    ...
    // restore env
    if (prev !== undefined) process.env.WHEN_DB_PATH = prev; else delete process.env.WHEN_DB_PATH;
  });

  it('creates parent directory when it does not exist (mkdir branch)', async () => {
    const tmpDir = path.join(process.cwd(), 'test-data', `isolated-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('does not attempt to create directory when it already exists (no-mkdir branch)', async () => {
    ...
    try { if (!fs.existsSync(existingDir)) fs.mkdirSync(existingDir, { recursive: true }); } catch {}
    ...
    try { if (db && typeof db.close === 'function') db.close(); } catch {}
    try { if (fs.existsSync(tempDb)) fs.unlinkSync(tempDb); } catch {}
  });
});
