...
describe("Ready listener", () => {
  ...
  it("closes poll when message fetch throws known discord errors", async () => {
    ...
    const channelA: any = {
      messages: { fetch: vi.fn().mockRejectedValue({ code: 10008 }) },
    };
    const clientA: any = {
      channels: { fetch: vi.fn().mockResolvedValue(channelA) },
    };
    ...
    const clientB: any = {
      channels: { fetch: vi.fn().mockResolvedValue(channelB) },
    };
    ...
    const clientC: any = {
      channels: { fetch: vi.fn().mockResolvedValue(channelC) },
    };
  ...
  it("constructs ReadyListener and leaves poll open when message exists", async () => {
    ...
    const channel: any = {
      messages: { fetch: vi.fn().mockResolvedValue({ id: "m1" }) },
    };
    const client: any = {
      channels: { fetch: vi.fn().mockResolvedValue(channel) },
    };
