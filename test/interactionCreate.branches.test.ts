...
describe("InteractionCreate additional branches", () => {
  ...
  it("handleLastSelect rejects when last < first", async () => {
    ...
    const interaction: any = {
      ...
      channel: {
        id: "chan-1",
        isTextBased: () => true,
        send: vi.fn().mockResolvedValue({ id: "m" }),
      },
    };
    ...
    const interaction: any = {
      ...
      fields: {
        getTextInputValue: (k: string) =>
          k === "first-date" ? "not-a-date" : "2025-08-31",
      },
      user: { id: "u-modal" },
      reply,
    };
  ...
  it("modal submit rejects when date range too large (>20) (consolidated)", async () => {
    vi.doMock("../src/util/date.js", () => ({
      ...
      buildDateRange: (_a: string, _b: string) =>
        Array.from(
          { length: 21 },
          (_, i) => `2025-09-${String(1 + i).padStart(2, "0")}`,
        ),
      buildFutureDates: (n: number) =>
        Array.from(
          { length: n },
          (_, i) => `2025-10-${String(1 + i).padStart(2, "0")}`,
        ),
    ...
    const ix: any = {
      ...
      fields: {
        getTextInputValue: (k: string) =>
          k === "first-date" ? "2025-09-01" : "2025-09-30",
      },
      user: { id: "u-modal2" },
      reply,
    };
  ...
  it("toggle replies Poll not found when poll id unknown (consolidated)", async () => {
    ...
    const ix: any = {
      isButton: () => true,
      customId: "when:toggle:missing:2025-08-30",
      user: { id: "u" },
      reply,
      update: vi.fn(),
    };
  ...
  it("toggleAll replies Poll not found when poll id unknown (consolidated)", async () => {
    ...
    const ix: any = {
      isButton: () => true,
      customId: "when:toggleAll:missing",
      user: { id: "u" },
      reply,
      update: vi.fn(),
    };
