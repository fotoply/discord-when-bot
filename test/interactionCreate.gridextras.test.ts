...
describe("InteractionCreate.buildGridExtras", () => {
  ...
  it("collects labels from member cache and fetches avatars via global fetch", async () => {
    ...
    const memberU2 = {
      ...
      user: {
        username: "bob",
        displayAvatarURL: () => "https://example.com/b.png",
      },
    };
  ...
  it("uses nickname/globalName fallbacks and skips avatar when fetch ok=false", async () => {
    ...
    const memberU4 = {
      nickname: "Nick4",
      user: { username: "u4", displayAvatarURL: () => "http://img/u4.png" },
    };
    const interaction: any = {
      guild: {
        members: {
          cache: new Map([["u4", memberU4]]),
          fetch: vi.fn().mockResolvedValue(memberU4),
        },
      },
      client: {
        users: {
          cache: new Map([
            [
              "u5",
              {
                globalName: "Global Five",
                username: "u5",
                displayAvatarURL: () => "http://img/u5.png",
              },
            ],
          ]),
          fetch: vi.fn(),
  ...
  it("resolves labels deterministically across many fallbacks", async () => {
    ...
    const member_m_disp = {
      ...
      user: {
        username: "md",
        globalName: "MD",
        displayAvatarURL: () => "http://a/md.png",
      },
    };
    const member_m_nick = {
      nickname: "MemberNick",
      user: { username: "mn", displayAvatarURL: () => "http://a/mn.png" },
    };
    const member_m_muglob = {
      user: {
        username: "mmu",
        globalName: "GlobalFromMember",
        displayAvatarURL: () => "http://a/mmu.png",
      },
    };

    // users: next three exist only at user level
    const user_user_disp = {
      displayName: "User Display",
      username: "ud",
      displayAvatarURL: () => "http://a/ud.png",
    };
    const user_user_glob = {
      globalName: "UserGlobal",
      username: "ug",
      displayAvatarURL: () => "http://a/ug.png",
    };
    const user_user_name = {
      username: "user_name",
      displayAvatarURL: () => "http://a/un.png",
    };
    ...
    const interaction: any = {
      guild: {
        members: {
          cache: new Map<string, any>([
            ["m_disp", member_m_disp],
            ["m_nick", member_m_nick],
            ["m_muglob", member_m_muglob],
          ]),
          fetch: vi.fn(),
        },
      },
      client: {
        users: {
          cache: new Map<string, any>([
            ["user_disp", user_user_disp],
            ["user_glob", user_user_glob],
  ...
  it("skips avatar fetch when displayAvatarURL is not a function", async () => {
    ...
    const member = {
      displayName: "NA",
      user: { username: "na1", displayAvatarURL: "http://not-a-fn" },
    };
    const interaction: any = {
      guild: {
        members: {
          cache: new Map<string, any>([["na1", member]]),
          fetch: vi.fn(),
        },
      },
      client: { users: { cache: new Map(), fetch: vi.fn() } },
  ...
  it("handles avatar fetch throwing exceptions gracefully (caught)", async () => {
    ...
    const member = {
      displayName: "U6",
      user: { username: "u6", displayAvatarURL: () => "http://img/u6.png" },
    };
    const interaction: any = {
