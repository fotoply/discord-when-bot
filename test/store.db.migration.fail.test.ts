...
describe('store/db migration failure path', () => {
  it('swallows errors when PRAGMA table_info fails', async () => {
    ...
    vi.doMock('better-sqlite3', () => {
      class FakeStmt {
        all() { throw new Error('pragma failed'); }
        run() { return; }
      }
      class FakeDB {
        constructor(_p: string) {}
        pragma(_q: string) { return 1 as any; }
        exec(_s: string) { return; }
        prepare(sql: string) { if (/PRAGMA\s+table_info/i.test(sql)) return new FakeStmt() as any; return new FakeStmt() as any; }
      }
