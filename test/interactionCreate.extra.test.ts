...
describe('InteractionCreate extra branches', () => {
    it('allows admin (non-creator) to close a poll', async () => {
        const poll = Polls.createPoll({channelId: 'c-admin', creatorId: 'orig', dates: ['2025-08-30']});
    ...
    it('replies when trying to close an already closed poll', async () => {
        const poll = Polls.createPoll({channelId: 'c-already', creatorId: 'creatorX', dates: ['2025-08-30']});
        Polls.close(poll.id);
    ...
    it('handles modal submit where followUp rejects (followUp.catch path)', async () => {
        ...
        const interaction: any = {
            ...
            fields: {
                getTextInputValue: (k: string) => (k === 'first-date' ? '2025-08-30' : '2025-08-31'),
            },
