...
describe("gridImage renderer (canvas path)", () => {
  ...
  it("draws gold in full-availability columns and respects bgColor", () => {
    ...
    expect([
      ...
      png.data[idxN + 2],
      png.data[idxN + 3],
    ]).toEqual([46, 204, 113, 255]);

    // top-left pixel should be background
    const idxBG = 0;
    expect([png.data[idxBG], png.data[idxBG + 1], png.data[idxBG + 2]]).toEqual(
      [0x11, 0x22, 0x33],
    );
  });
  ...
  it("places a background pixel before the first cell due to extra space after label", () => {
    ...
    expect([
      png.data[idxBefore],
      png.data[idxBefore + 1],
      png.data[idxBefore + 2],
      png.data[idxBefore + 3],
    ]).toEqual([0, 0, 0, 255]);
  });
  ...
  it("honors very long row label and expands rowLabelWidth accordingly", () => {
    const longLabel =
      "ThisIsAVeryLongLabelThatShouldIncreaseMeasuredWidthSignificantly";
    const matrix = [[true]];
  ...
  it("does not throw when WHEN_REQUIRE_CANVAS=1 and canvas is available", () => {
    ...
    try {
      ...
      expect(() =>
        renderGridPng([[true]], { rowLabels: ["A"] } as any),
      ).not.toThrow();
    } finally {
  ...
  it("handles very narrow character measurement (spaceWidthZero) combined with charScale variations", () => {
    ...
    __setCanvasModule(
      makeFakeCanvasModule({ spaceWidthZero: true, charScale: 0.2 }),
    );
    const matrix = [[true, false]];
    ...
    __setCanvasModule(
      makeFakeCanvasModule({ spaceWidthZero: false, charScale: 1.5 }),
    );
    const { width: w2 } = renderGridPng(matrix, {
      rowLabels: ["LONG LABEL EXAMPLE"],
    });
