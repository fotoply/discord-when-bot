...
describe("Poll command additional coverage", () => {
  it("registerApplicationCommands wires context menu builder and invokes option callbacks", async () => {
    ...
    await PollCommandClass.prototype.registerApplicationCommands.call(
      { name: "poll", description: "d" } as any,
      registry as any,
    );
  ...
  it("context menu success path uses editReply when deferred", async () => {
    const poll = Polls.createPoll({
      channelId: "chan-def-succ",
      creatorId: "creatorDef",
      dates: ["2025-09-02"],
    });
  ...
  it("contextMenuRun delegates to messageRun", async () => {
    ...
    const spy = vi
      .spyOn(PollCommand.prototype as any, "messageRun")
  ...
  it("/poll repost skips deletion when old message not found", async () => {
    const poll = Polls.createPoll({
      channelId: "chan-old-null",
      creatorId: "creatorOldNull",
      dates: ["2025-09-03"],
    });
    ...
    const destChannel = {
      id: "dest-after-null",
      isTextBased: () => true,
      send: vi.fn().mockResolvedValue(newMsg),
    };

    const fakeCmd: any = {
      container: { client: { channels: { fetch: channelsFetch } } },
    };
    const interaction: any = {
      options: {
        getSubcommand: () => "repost",
  ...
  it("updates original message via interaction.client when no container", async () => {
    ...
    const oldChannel = {
      isTextBased: () => true,
      messages: { fetch: vi.fn().mockResolvedValue(oldMsg) },
    };
