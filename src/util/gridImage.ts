...
export function __setCanvasModule(mod: any) { CanvasOverride = mod; }

function getCanvasMod(): any {
    if (CanvasOverride !== undefined) return CanvasOverride;
    if (typeof CanvasMod === 'undefined') {
        try { CanvasMod = requireCjs('canvas'); } catch { CanvasMod = null; }
    }
    return CanvasMod;
}
...
export function renderGridPng(matrix: boolean[][], opts: GridImageOptions = {}): { buffer: NodeBuffer; width: number; height: number } {
    ...
    if (!Canvas?.createCanvas) {
        throw new Error('node-canvas is required but not available. Ensure Node 20 and install canvas.');
    }
    return renderWithCanvas(matrix, opts, Canvas);
}

function renderWithCanvas(matrix: boolean[][], opts: GridImageOptions, CanvasLib: any): { buffer: NodeBuffer; width: number; height: number } {
    ...
    const rowLabelWidth = opts.rowLabelWidth ?? Math.max(240, computedRowLabelWidth);
