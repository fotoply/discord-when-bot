  ...
  public async run(client: Client, pollsModule?: any) {
    ...
    try {
      ...
      for (const poll of open) {
        console.log(
          `Verifying poll ${poll.id} (channel=${poll.channelId} message=${poll.messageId} closed=${poll.closed})`,
        );
        if (!poll.messageId) {
        ...
        try {
          ...
          if (!channel || !("messages" in channel)) {
            ...
            Polls.close(poll.id);
            console.log(
              `Closed poll ${poll.id} because channel was not found or cannot hold messages.`,
            );
            continue;
          }
          ...
          if (
            ...
            code === 10003 ||
            (err?.message &&
              /Unknown Message|Unknown Channel/i.test(err.message))
          ) {
            Polls.close(poll.id);
            console.log(
              `Closed poll ${poll.id} because message or channel was deleted.`,
            );
          } else {
