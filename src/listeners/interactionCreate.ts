...
import { Polls, type Poll } from '../store/polls.js';
...
export default class InteractionCreateListener extends Listener<typeof Events.InteractionCreate> {
  public constructor(context: Listener.Context, options: Listener.Options = {}) {
    super(context, { ...options, event: Events.InteractionCreate });
  }

  public async run(interaction: Interaction) {
    if (interaction.isModalSubmit() && interaction.customId === 'when:date-range') {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'when:first') {
      await this.handleFirstSelect(interaction);
      return;
    }

    if (interaction.isStringSelectMenu() && interaction.customId === 'when:last') {
  ...
  private async handleDateRangeModal(interaction: any) {
    ...
    if (!isValidISODate(firstRaw) || !isValidISODate(lastRaw)) {
      await interaction.reply({ content: 'Please use valid dates in the form YYYY-MM-DD.', ephemeral: true });
      return;
    }

    const dates = buildDateRange(firstRaw, lastRaw);
    if (!dates) {
      await interaction.reply({ content: 'First date must be on or before last date.', ephemeral: true });
      return;
    }

    if (dates.length === 0) {
      await interaction.reply({ content: 'No dates in range.', ephemeral: true });
      return;
    }

    if (dates.length > 20) {
      await interaction.reply({ content: 'Date range too large. Please choose 20 days or fewer.', ephemeral: true });
      return;
    }

    const poll = Polls.createPoll({ channelId: interaction.channelId ?? 'unknown', creatorId: interaction.user.id, dates });

    const rows = this.componentsFor(poll);

    await interaction.reply({ content: this.renderPollContent(poll), components: rows });

    const message = await interaction.fetchReply();
    Polls.setMessageId(poll.id, message.id);

    await interaction.followUp({ content: 'Poll created!', ephemeral: true }).catch(() => {});
  }
  ...
  private async handleFirstSelect(interaction: StringSelectMenuInteraction) {
    ...
    const firstSelect = new StringSelectMenuBuilder()
      ...
      .setMinValues(1)
      .setMaxValues(1)
      .addOptions(future.map((iso) => ({ label: formatDateLabel(iso), value: iso, default: iso === first })));
    ...
    const lastSelect = new StringSelectMenuBuilder()
      ...
      .setMaxValues(1)
      .setDisabled(false)
      .addOptions(filtered.map((iso) => ({ label: formatDateLabel(iso), value: iso })));
  ...
  private async handleLastSelect(interaction: StringSelectMenuInteraction) {
    ...
    if (!first) {
      await interaction.reply({ content: 'Please pick the first date first.', ephemeral: true });
      return;
    }

    if (!last || last < first) {
      await interaction.reply({ content: 'Last date must be the same or after the first date.', ephemeral: true });
      return;
    }
    ...
    if (!interaction.inGuild() || !interaction.channel?.isTextBased()) {
      await interaction.reply({ content: 'Cannot determine a text channel to post in.', ephemeral: true });
      return;
    }

    const poll = Polls.createPoll({ channelId: interaction.channel.id, creatorId: interaction.user.id, dates });

    const rows = this.componentsFor(poll);

    const message = await interaction.channel.send({ content: this.renderPollContent(poll), components: rows });
  ...
  private async handleToggle(interaction: ButtonInteraction) {
    ...
    if (!pollId || !date) {
      await interaction.reply({ content: 'Invalid button payload.', ephemeral: true });
      return;
    }

    const res = Polls.toggle(poll.id, date, interaction.user.id);
    if (!res) {
      await interaction.reply({ content: 'Poll not found or invalid date.', ephemeral: true });
      return;
    }

    const updated = Polls.get(poll.id)!;

    await interaction.update({ content: this.renderPollContent(updated), components: this.componentsFor(updated) });
  }
  ...
  private async handleToggleAll(interaction: ButtonInteraction) {
    ...
    if (!res) {
      await interaction.reply({ content: 'Could not toggle all.', ephemeral: true });
      return;
    }
  ...
  private async handleClose(interaction: ButtonInteraction) {
    ...
    if (interaction.user.id !== poll.creatorId) {
      await interaction.reply({ content: 'Only the poll creator can close this poll.', ephemeral: true });
      return;
    }
    ...
    await interaction.update({ content: this.renderPollContent(updated), components: [] });
  }
  ...
  private componentsFor(poll: Poll): ActionRowBuilder<ButtonBuilder>[] {
    ...
    for (const set of poll.selections.values()) for (const u of set) voters.add(u);
    ...
    for (const d of poll.dates) {
      ...
      const allOk = votersCount > 0 && count === votersCount;
      const labelBase = `${formatDateLabel(d)} (${count})`;
    ...
    const controls = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(`when:toggleAll:${poll.id}`).setLabel('Toggle all').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`when:close:${poll.id}`).setLabel('Close poll').setStyle(ButtonStyle.Danger)
    );
    rows.push(controls);
  ...
  private renderPollContent(poll: Poll): string {
    const lines: string[] = [];
    const header = poll.closed ? `Availability poll by <@${poll.creatorId}> — CLOSED` : `Availability poll by <@${poll.creatorId}>. Click the dates you are available:`;
    lines.push(header);
    lines.push('');
    ...
    for (const d of poll.dates) {
      ...
      const allOk = votersCount > 0 && set.size === votersCount;
      const star = allOk ? '⭐ ' : '';
      lines.push(`• ${star}${formatDateLabel(d)} — ${who}`);
    }

    const votersLine = [...voters].map((u) => `<@${u}>`).join(', ') || '-';
    lines.push('');
